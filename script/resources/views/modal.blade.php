...
<div id="leftarrow"></div>
    <div class="row">
        <div class="col-lg-7 p-0">
            ...
            <div id="proflink">
                <a href="{{ route('profile.show',$video->user->slug) }}" class="volume"><img src="{{ asset('frontend/img/profile.png') }}"></a>
            </div> 
            <div class="loader">
            ...
            <div class="modal-right-section">
                <div class="user-top-info">
                    <div class="user-profile-info">
                        <a class="pjax" id="profilelink" href="{{ route('profile.show',$video->user->slug) }}" onclick="profileshow()"> <img src="{{ asset($video->user->image) }}" alt=""> {{ $video->user->username }}</a>
                    </div>
                ...
                <div class="modal-comment-list">
					<h5 id="vidtitleh5">{{ $video->title }}</h5>
                    ...
                    <div class="single-comment ml-50">
                        ...
                        @if($value->user->fire)
                        <img src="/backend/assets/img/achievments/fire.svg" class="achievscommentava">
                        @elseif($value->user->check)
                        <img src="/backend/assets/img/achievments/check.svg" class="achievscommentava">
                        @endif
                        <span> 
                            <a class="pjax" href="{{ route('profile.show',$value->user->slug) }}" onclick="profileshow()">{{ $value->user->username }}</a>
							@if($value->user->heart)
							<img src="/backend/assets/img/achievments/heart.svg" class="achievscommentlogin">
							@elseif($value->user->diamond)
							<img src="/backend/assets/img/achievments/diamond.svg" class="achievscommentlogin">
							@endif
							<br><a class="username" href="{{ $value->mention_id != null ? route('profile.show',$value->mention_user->slug) : '' }}" onclick="profileshow()">{{ $value->mention_id != null ? $value->mention_user->username : '' }}</a>{{ $value->message }} 
                            <div class="comment-info">
                ...
                <div class="send-comment-area">
                    ...
                    <form action="{{ route('comment') }}" method="POST" id="comment_send">
                        ...
                        <div class="d-flex">
                            ...
                            <button type="submit" id="postcommbtn">{{ __('Post') }}</button>
                        </div>
                    </form>
                    @else
