...
<div class="single-comment ml-50">
	...
	@endif
    <span> 
        <a class="pjax" href="{{ route('profile.show',$value->user->slug) }}" onclick="profileshow()">{{ $value->user->username }}</a><br><a class="username" href="{{ $value->mention_id != null ? route('profile.show',$value->mention_user->slug) : '' }}" onclick="profileshow()">{{ $value->mention_id != null ? $value->mention_user->username : '' }}</a>
		@if($value->user->heart)
		<img src="/backend/assets/img/achievments/heart.svg" class="achievscommentlogin">
