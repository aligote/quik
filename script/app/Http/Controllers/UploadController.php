...
class UploadController extends Controller
{
    ...
    public function index()
    {
		...
		if (!$verification OR $verification->status != 'approved')
            return redirect()->to('/settings?page=verify');
        $option = Option::where('key','user_value')->first();
    ...
    public function upload(Request $request)
    {
        $validator = \Validator::make($request->all(), [
            'video' => 'required|mimetypes:video/mp4,video/quicktime|max:204800'
        ]);
    ...
    public function store(Request $request)
    {
		...
		if (mb_strlen($request->caption) < 5 OR mb_strlen($request->caption) > 150)
			return response()->json(['errors' => 'Length of Caption must be from 5 to 150 characters.']);
