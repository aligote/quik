(function($) {
	...
	$(document).on('pjax:complete',function(){
		...
		$('#selfieimage').on('change',function(){
			var reader = new FileReader();
			reader.onload = function (e) {
				$("#selfieimage_card").css("background-image", "url("+e.target.result+")");
				console.log(e.target.result);
				$("#selfieimage_card svg").fadeOut();
			}
			reader.readAsDataURL(event.target.files[0]);
		});
		$('#secondimage').on('change',function(){
			var reader = new FileReader();
			reader.onload = function (e) {
				$("#secondimage_card").css("background-image", "url("+e.target.result+")");
				console.log(e.target.result);
				$("#secondimage_card svg").fadeOut();
			}
			reader.readAsDataURL(event.target.files[0]);
		});
		$('#pasportidmage').on('change',function(){
			var reader = new FileReader();
			reader.onload = function (e) {
				$("#verifyimage_card").css("background-image", "url("+e.target.result+")");
				console.log(e.target.result);
				$("#verifyimage_card svg").fadeOut();
			}
			reader.readAsDataURL(event.target.files[0]);
		});
	});
